...
const Chart = () => {
  ...
  async function candle_price(coin, intervals, length) {
    const api_url =
      "https://api.bithumb.com/public/candlestick/" +
      coin +
      "_KRW/" +
      intervals;
    ...
    async function DataRenew() {
      dbService
        ...
        .then((snapshot) => {
          ...
          const initAsset =
            snapshotReverse[0].data().btc +
            snapshotReverse[0].data().eth +
            snapshotReverse[0].data().bnb +
            snapshotReverse[0].data().cash;
      ...
      const composePrice = btcObj.price.map((x, y) =>
        Number((x + ethObj.price[y] + bnbObj.price[y]) / 3).toFixed(2)
      );
      ...
      <FlexBox>
        ...
        <Checkbox>
          <input
            ...
            onChange={showbcompositeClicked}
          />
